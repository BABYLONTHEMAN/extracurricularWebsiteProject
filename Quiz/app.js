...
let currentQuestion = Math.floor(Math.random() * quiz.length); //am let'a bakar yat bo call'krdnaway prsyary 1 bo 5
...
let randomQuestion = quiz[randomIndex].question;
let currentIterate = 6; 

function showQuestion() {
//-_-_-_-_-_-_-_-_-__-_-_-_-_-_-_-_-_-_-_-_-_-_-
//________________declarations  
...
//-_-_-_-_-_-_-_-_-__-_-_-_-_-_-_-_-_-_-_-_-_-_-
    ...
    for (let i = 0; i < q.choices.length; i++) {
        ...
        btn.addEventListener("click", function() {
            ...
            myChoice = btn;
            myChoice.style.backgroundColor = "powderblue";
            select.innerHTML = "";
        });
...
function checkAnswer(answer) {
    daium();
    const q = currentQuestion; //q = quiz[currentQuestion] wata q = (prsyary 1)            |
        //window.alert(`Question ${i + 1}: ${randomQuestion}`)
...
for (let i = 0; i < numIterations; i++) {
  ...
  const randomQuestion = quiz[randomIndex].question;
}
*/
showQuestion();